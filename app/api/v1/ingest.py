...
def ingest_company_listings(rows: list[CompanyListingIngest], db: Session = Depends(get_db)) -> IngestResult:
    for index, row in enumerate(rows):
        try:
            ...
            db.rollback()
            raise _db_error(409, "company-listings", index, row.corp_code, error) from error
        except SQLAlchemyError as error:
            db.rollback()
            raise _db_error(500, "company-listings", index, row.corp_code, error) from error
    db.commit()
    return IngestResult(received=len(rows), inserted_or_updated=len(rows))
...
def ingest_dart_filings(rows: list[DartFilingIngest], db: Session = Depends(get_db)) -> IngestResult:
    for index, row in enumerate(rows):
        try:
            ...
            db.rollback()
            raise _db_error(409, "dart-filings", index, row.corp_code, error) from error
        except SQLAlchemyError as error:
            db.rollback()
            raise _db_error(500, "dart-filings", index, row.corp_code, error) from error
    db.commit()
    return IngestResult(received=len(rows), inserted_or_updated=len(rows))
...
def ingest_financial_statement_items(
    ...
    for index, row in enumerate(rows):
        try:
            ...
            db.rollback()
            raise _db_error(409, "financial-statement-items", index, row.corp_code, error) from error
        except SQLAlchemyError as error:
            db.rollback()
            raise _db_error(500, "financial-statement-items", index, row.corp_code, error) from error
    db.commit()
    return IngestResult(received=len(rows), inserted_or_updated=len(rows))
